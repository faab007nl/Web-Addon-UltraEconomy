...
<head>
    ...
    <title><?php echo $lang->accounts; ?> - Ultra Economy</title>
        ...
        <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0">
            <div class="container-fluid d-flex flex-column p-0"><a class="navbar-brand d-flex justify-content-center align-items-center sidebar-brand m-0"><img class="img-fluid MenuHeader" src="assets/img/uecon.png"></a>
                <ul class="nav navbar-nav text-light" id="accordionSidebar">
                    <li class="nav-item"><a class="nav-link active" href="./accounts"><i class="fas fa-users"></i><span><?php echo $lang->accounts; ?></span></a></li>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex justify-content-between align-items-center mb-4">
                        <h3 class="text-dark mb-0"><?php echo $lang->accounts; ?></h3>
                    </div>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th><?php echo $lang->balancebank; ?></th>
                                    <th><?php echo $lang->balancehand; ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                    $pdoResult1 = $PDOdb->prepare("SELECT * FROM ultraeconomy_accounts ORDER BY ultraeconomy_accounts.key DESC");
                                    ...
                                    if($pdoExec1){
                                        while($row1 = $pdoResult1->fetch(PDO::FETCH_ASSOC)){
                                            ...
                                            foreach ($balances as $currencie => $balance) {
                                                $pdoResult2 = $PDOdb->prepare("SELECT * FROM ultraeconomy_currencies WHERE ultraeconomy_currencies.key=:key");
                                                $pdoExec2 =  $pdoResult2->execute(array(":key"=>$currencie));
                                                
                                                if($pdoExec2){
                                                    while($row2 = $pdoResult2->fetch(PDO::FETCH_ASSOC)){
                                                        $data2 = base64_decode($row2['value']);
                                                        $data2 = json_decode($data2, true);
                                                        $currencieName = $data2['name'];
                                                    }
                                                }else{
                                                    echo 'error getting currencie ';
                                                }

                                                $Bank .= "<span>".$currencieName.": ".$balance['bank']."</span><br>";
                                                $Hand .= "<span>".$currencieName.": ".$balance['hand']."</span><br>";
                                            }

                                            echo '
                                                <tr>
                                                    <td class="table-cell">'.$name.'</td>
                                                    <td class="table-cell">'.$Bank.'</td>
                                                    <td class="table-cell">'.$Hand.'</td>
                                                </tr>
                                            ';
                                        }
                                    }else{
